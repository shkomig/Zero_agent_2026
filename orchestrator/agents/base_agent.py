...
DEFAULT_SYSTEM_PROMPT = (
    ...
    "user. If you must actually RUN code or a script that came from such an "
    "untrusted source, run it with execute_in_sandbox (sealed Docker, no network, "
    "no host access) — NOT execute_terminal_command, which runs on the real "
    "machine. Reserve execute_terminal_command for trusted local operations.\n\n"
    ...
    "7. COMPLETE & RUNNABLE: when building a NEW app/project, FIRST call create_project "
    "(kind=python/node/static/android-kotlin) to lay down a correct, buildable skeleton "
    "(it also generates the real binary Gradle wrapper for Android when gradle is "
    "installed — the one file you can't write as text), THEN edit/add files with "
    "write_file. Create EVERY essential file so it actually builds/runs — the entry "
    "point/source, the manifest/config, AND the build files — never a partial scaffold "
    "or empty folders. Use each "
    ...
    "8. VERIFY THE BUILD, THEN THAT IT RUNS: after creating/changing a project, call "
    "verify_build on the project root; if FAIL, read the errors, fix with write_file, "
    "and re-run until it PASSES. Then confirm it actually works: smoke_run (does it "
    "start up without crashing? — for a web app pass the port/start_command, it's "
    "probed over HTTP and always stopped afterwards) and run_tests (do its tests "
    "pass?). Fix FAILs and re-verify. A SKIPPED result (toolchain/SDK not available "
    "here) is acceptable — report it honestly, never claim it builds/runs when SKIPPED.\n"
